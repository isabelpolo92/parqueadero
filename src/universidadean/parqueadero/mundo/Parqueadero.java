...
public class Parqueadero {
    ...
    public String metodo1() {
    	...
    	HashMap<String, Integer> hashCarros = new HashMap<>();
    	
        List<Integer> listaCarros = new ArrayList<>();
        int puestosLibres = 0;
        for (Puesto puesto : puestos) {
            if (puesto.estaOcupado()) {
            	hashCarros.put(puesto.darCarro().darPlaca(), puesto.darCarro().darTiempoEnParqueadero(horaActual)); //.put(puesto.darCarro().darTiempoEnParqueadero(horaActual)); 
            }
        }
        
        int max = Collections.max(hashCarros.values());
    	
    	List<String> list =  hashCarros.entrySet().stream()
        .filter(entry -> entry.getValue() == max)
        .map(entry -> entry.getKey())
        .collect(Collectors.toList());
    	
        return "El carro que lleva más tiempo en el parqueadero es "+ list.get(0) + " y lleva las siguiente cantidad de horas "+max;
    }
    ...
    public String metodo2() {
    	...
    	String response = "Hay puestos disponibles";
    	
    	if(puestosDesocupados==0) {
    		response = "El parqueadero esta lleno";
    	}
